...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # Mask out-of-vocab tokens if logits are larger than tokenizer
        # vocab_size. This prevents sampling tokens that the tokenizer
        # cannot handle. Only mask if we successfully determined
        # tokenizer_vocab_size and the logits actually extend beyond it.
        if (self.tokenizer_vocab_size is not None
                and logits.shape[-1] > self.tokenizer_vocab_size):
